...
lb_thetaUE = thetaUE - qt(0.975,df=length(bX1)-2)\*se_thetaUE.random ub_thetaUE = thetaUE + qt(0.975,df=length(bX1)-2)\*se_thetaUE.random p_thetaUE = 2\*(1-pt(abs(thetaUE/se_thetaUE.random),df=length(bX1)-2)) #Test for directional pleiotropy interUE = summary(lm(bY\~bX1, weights=bYse\^-2))\$coef[1] se_interUE.random = summary(lm(bY\~bX1, weights=bYse\^-2))$coef[1,2]/ min(summary(lm(bY~bX1, weights=bYse^-2))$sigma,1) p_interUE = 2(1-pt(abs(interUE/se_interUE.random),df=length(bX1)-2))
