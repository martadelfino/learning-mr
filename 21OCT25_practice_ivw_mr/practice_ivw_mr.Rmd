...
-   The Outcome file is ready.

-   The Exposure file needs cleaning. Selecting columns and creating new SNP IDs with chr number and position. Selecting only rows with positive betas - just make a new column for this. Flipping the ones with negative betas - make a new column for these as well.
...
library(tidyverse)
 ...
 exposure_raw <- readxl::read_excel("41588_2024_1798_MOESM4_ESM.xlsx", sheet = "ST2 | 1080 signals")
...
write_tsv(exposure, "exposure_menarche_timing.tsv")
```

Reading the files

```{r}
exposure <- read.delim("exposure_menarche_timing.tsv", 
                      sep = "\t",
                      stringsAsFactors = FALSE)
outcome <- read.delim("Mahajan.NatGenet2018b.T2D.European.txt", 
                      sep = "\t",
                      stringsAsFactors = FALSE)
```
...
exposure_mr <- exposure %>%
  dplyr::select(SNP, ALLELE1, ALLELE0, Beta_positive, SE) %>%
  dplyr::filter(!is.na(Beta_positive)) %>%
  dplyr::rename(
    beta_exposure = Beta_positive,
    se_exposure = SE,
    ea_exposure = ALLELE1,
...
Merging - only doing MR on SNPs that exist in both

```{r}
merged_check <- inner_join(exposure_mr, outcome_mr, by = "SNP") %>%
  dplyr::mutate(
    # Identify matching or reversed alleles
    allele_match = case_when(
      ea_exposure == ea_outcome & nea_exposure == nea_outcome ~ "match",
      ea_exposure == nea_outcome & nea_exposure == ea_outcome ~ "reverse",
      TRUE ~ "mismatch"
    ))

merged <- merged_check %>% dplyr::filter(allele_match != "mismatch")
...
```{r}
# IVW point estimate
theta_IVW <- sum(merged$beta_outcome * merged$beta_exposure * merged$weight) /
...
ggplot(merged, aes(x = beta_exposure, y = beta_outcome)) +
...
ivw_model <- lm(beta_outcome ~ beta_exposure - 1,
                data = merged,
